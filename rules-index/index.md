# rules-index
> a list of all the pages of rules and guidelines available for THAUMWORLD

they are listed under the extact keyword that titles their folder and index.md file.
these are broken into sections for ease of parsing. higher sections are more base systems of the game, while lower sections are more specific rulings or uncommon but neccessary pages.

---
## meta-pages

### landing-page
> the first page of THAUMWORLD, showing what it is.
<https://www.thaumworld.xyz/>

### rules-index 
> a list of all the pages of rules and guidelines available for THAUMWORLD. this is the current page 
<https://www.thaumworld.xyz/rules-index/>

### index-files-rules-n-structure
> some breakdowns of how the index.md files and the markdown on them should be formatted
<https://www.thaumworld.xyz/index-files-rules-n-structure/>


---
## core-mechanics
> these are all the rules that are the essential pieces of the engine.

### stats-n-bonuses
> what makes a character
<https://www.thaumworld.xyz/stats-n-bonuses/>

### magnitude
> rolls, damage, hardness, conductivity, weight, size, temparature, speed, and time at scale
<https://www.thaumworld.xyz/magnitude/>

### dice-rolls-n-nat
> defining uncertainty
<https://www.thaumworld.xyz/dice-rolls-n-nat/>

### reroll
> collective minimum or maximums from multiple rolls
<https://www.thaumworld.xyz/rerolls/>

### result-rolls 
> how to tell the result of something
<https://www.thaumworld.xyz/result-rolls/>

### challenge-peak-n-blunder-ratings
> extraordinary success and failure of result rolls
<https://www.thaumworld.xyz/challenge-peak-n-blunder-ratings/>

### potency-rolls
> how to tell how potent or powerful something is
<https://www.thaumworld.xyz/potency-rolls/>

### character
> anything that can do actions
<https://www.thaumworld.xyz/character/>

### actions
> how characters do things
<https://www.thaumworld.xyz/actions/>

### effectors
> how numbers can change
<https://www.thaumworld.xyz/effectors/>

### tags
> special ammendments to the rules 
<https://www.thaumworld.xyz/tags/>

### tag-table
> all the tags that exist
<https://www.thaumworld.xyz/tag-table/>

### conditions
> observations of things
<https://www.thaumworld.xyz/conditions/>

---

## the role of the STORY TELLER
> this is my process of being the STORY TELLER broken down to a programatic level
> if you are an ACTOR, this is like reading how the engine works on the STORY TELLERs side. 
> you do not have to know these rules and the story teller is the only one required to know these. 
> some of these pages are design thoughts, examples, and me talking about my process as the creator, J

### story
> has beats that happen, and a flow that controls those beats
<https://www.thaumworld.xyz/story/>

### story-beat-n-archetype
> an event that happens along a story
<https://www.thaumworld.xyz/story-beat-n-archetype/>

### story-teller
> the person in charge of creating, and telling the STORY of the game with the ACTORS
<https://www.thaumworld.xyz/story-teller/>

### dissonance-n-harmony
> a budget to add positive and negative modifiers to the content of a STORY BEAT
<https://www.thaumworld.xyz/dissonance-n-harmony/>

### story-flow
> the mapping of harmony and dissonance of a story beats along a story
<https://www.thaumworld.xyz/story-flow/>

### archetype-table
> a list of scalable contents for story-beats
<https://www.thaumworld.xyz/archetype-table/>

### story-beat-content
> what happens during a story beat, set by the archetype table and modulated with MAG
<https://www.thaumworld.xyz/story-beat-content/>



---

## txt-data-storage
> data storage for world tiles, characters, and items. 
> this section is about how i use text to store things so they can be parsed by ai assistants




---

## time-based-mechanics
> this is rules specifically for timed based events and very precisies moving

### timed_events-initiative-n-turns
> how time flows in sensitive moments
<https://www.thaumworld.xyz/timed_events-initiative-n-turns/>

### awareness
> if a character knows your there
<https://www.thaumworld.xyz/awareness/>



--- 
## tile-mechanics

### tiles-n-occupied
> the world as a grid of voxels
<https://www.thaumworld.xyz/tiles-n-occupied/>

### world-tiles
> like tiles but for moving around large regions instead of tight spaces
<https://www.thaumworld.xyz/world-tiles/>



--- 
## character-mechanics

### proficiency
> a measurement of a characters specified knowledge or abilities 
<https://www.thaumworld.xyz/proficiency/>

### perks
> what makes characters different
<https://www.thaumworld.xyz/perks/>

### movement
> an action type, and how you get around
<https://www.thaumworld.xyz/movement/>

### vigor 
> on a scale of negative 1 to infinity how alive are you?
<https://www.thaumworld.xyz/vigor/>

### health
> how alive you are from not getting stabbed
<https://www.thaumworld.xyz/health/>

### senses
> how CHARACTERs become AWARE of the world
<https://www.thaumworld.xyz/senses/>


### diet
> the resources someone needs to sustain life
<https://www.thaumworld.xyz/diet/>

### damage
> harmful changes to health, vigor, or thaum
<https://www.thaumworld.xyz/damage/>

### evasion
> how characters avoid being hit
<https://www.thaumworld.xyz/evasion/>

### fortitude
>how characters avoid being mentally hit
<https://www.thaumworld.xyz/fortitude/>

### sleep-n-repair
> action types that heal you
<https://www.thaumworld.xyz/sleep-n-repair/>

### safe-n-not-safe-temeratures
> what happens when things get too hot or too cold
<https://www.thaumworld.xyz/safe-n-not-safe-temeratures/>

### death 
> the end of a CHARACTER
<https://www.thaumworld.xyz/death/>

### body-slots-n-equipt
> how you equipt items
<https://www.thaumworld.xyz/body-slots-n-equipt/>

### weight-penalty
> when your collective items slow you down
<https://www.thaumworld.xyz/weight-penalty/>



---
## item-mechanics

### items-n-inventory
> for picking things up
<https://www.thaumworld.xyz/items-n-inventory/>

### tools
> items that that empower actions through tags
<https://www.thaumworld.xyz/tools/>

### armors
> items that that empower characters through tags
<https://www.thaumworld.xyz/armors/>

### break-threshold
> when something becomes broken
<https://www.thaumworld.xyz/break-threshold/>



---

## spell-mechanics

### spell-casting
> how spells are executed
<https://www.thaumworld.xyz/spell-casting/>

### spell-slots
> how many spells a character knows at one time
<https://www.thaumworld.xyz/spell-slots/>

### glyphs-n-calls
> are to words, as spells are to sentences
<https://www.thaumworld.xyz/glyphs-n-calls/>

---





